...
//             <ProvideCard key={index} icon={item.icon} title={item.title} description={item.description} />
//           ))}
//         </div>
//       </div>
//     </div>
...
// Updated ProvideCard component for enhanced hover effects
const ProvideCard = ({ icon: Icon, title, description }) => (
  <div
    ...
    // - hover:shadow-2xl: Increases shadow size on hover for more depth.
    // - hover:scale-[1.03]: Slightly enlarges the card on hover.
    // - hover:-translate-y-2: Lifts the card slightly on hover.
    // - transform: Added to base classes to ensure smooth transition for transform properties.
    // - transition-all duration-300 ease-in-out: Applies a smooth transition to all animatable properties.
    // - cursor-pointer: Changes mouse cursor to indicate it's clickable/interactive.
  >
    <div className="flex justify-center mb-4">
      <Icon className="h-12 w-12 text-purple-600" />
    </div>
...
const WhatWeProvide = () => {
  ...
  const titleVariants = {
    hidden: { opacity: 0, y: -30 }, // Fades in and slides down slightly
    visible: {
      opacity: 1,
      y: 0,
