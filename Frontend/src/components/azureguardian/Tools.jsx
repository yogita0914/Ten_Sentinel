...
// // ToolCard component modified for hover reveal
// const ToolCard = ({
//   logoComponent: Logo,
//   name,
//   description,
...
//     >
//       <div
//         className={`absolute inset-0 flex flex-col justify-center items-center transition-opacity duration-300 ease-in-out ${
...
//       </div>
//       <div
//         className={`absolute inset-0 bg-indigo-100 p-5 flex flex-col justify-center items-center text-center transition-all duration-300 ease-in-out transform ${
//           isRevealed
...
//   }, [itemWidth]); // itemWidth dependency might cause loops if not careful, but seems okay here with the NaN check
...
//               }}
//               // Add onMouseLeave to clear hoveredCardId if mouse leaves the entire carousel track
//               // This prevents a card from staying "open" if the mouse exits the track from that card
...
//                     onMouseLeave={() => setHoveredCardId(null)} // This will clear when mouse leaves individual card
//                   />
//                 </div>
//               ))}
//             </div>
...
//     >
//       <div
//         className={`absolute inset-0 flex flex-col justify-center items-center ${ // Removed transition classes for instant reveal
//           isRevealed ? "opacity-0" : "opacity-100"
//         }`}
//       >
//         <div className="bg-white rounded-lg p-4 mb-3 shadow-md w-24 h-24 flex items-center justify-center">
...
//       </div>
//       <div
//         className={`absolute inset-0 bg-indigo-100 p-5 flex flex-col justify-center items-center text-center transform ${ // Removed transition classes for instant reveal
//           isRevealed
//             ? "translate-y-0 opacity-100"
//             : "translate-y-full opacity-0"
//         }`}
...
//         if (isNaN(calculatedItemWidth) || calculatedItemWidth <= 0) {
//           // Fallback to previous itemWidth if calculation is problematic
//           // This check helps prevent issues if containerWidth is 0 temporarily
//           // but it's important itemWidth has a sane initial value.
//           calculatedItemWidth = itemWidth; // Using the existing itemWidth state as a fallback
//         }
//         setItemWidth(calculatedItemWidth);
//       }
//     };
...
//     window.addEventListener("resize", calculateLayout);
//     return () => window.removeEventListener("resize", calculateLayout);
//   }, [itemWidth]); // itemWidth dependency is kept, ensure initial state and logic handle it.
...
// ToolCard component (no changes needed here from your last version,
// as it will simply not receive the onMouseLeave prop if not passed)
const ToolCard = ({
  logoComponent: Logo,
  name,
  description,
  isRevealed,
  onMouseEnter,
  onMouseLeave, // This prop might be undefined if not passed, which is fine.
}) => {
  return (
    <div
      className="relative bg-teal-500 rounded-xl shadow-lg h-60 md:h-64 flex flex-col justify-center items-center p-4 overflow-hidden group w-full"
      onMouseEnter={onMouseEnter}
      onMouseLeave={onMouseLeave} // If onMouseLeave is undefined, this does nothing.
    >
      <div
        ...
        <div className="relative">
          ...
          <div className="overflow-hidden" ref={viewportRef}>
            <div
              ...
              // This onMouseLeave on the track is important for clearing hover when leaving the carousel area
              onMouseLeave={() => setHoveredCardId(null)}
            >
              {toolsData.map((tool, index) => (
                <div
                ...
                >
                  <ToolCard
                    ...
                    isRevealed={hoveredCardId === tool.id}
                    onMouseEnter={() => setHoveredCardId(tool.id)}
                    // The onMouseLeave prop that was here before:
                    // onMouseLeave={() => setHoveredCardId(null)}
                    // is now REMOVED from the individual ToolCard.
                    // The ToolCard component's onMouseLeave prop will be undefined,
                    // so no onMouseLeave handler will be attached to the individual card's div.
                  />
                </div>
              ))}
            </div>
