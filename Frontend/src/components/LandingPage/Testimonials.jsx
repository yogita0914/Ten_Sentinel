      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="relative h-[320px] sm:h-[360px] md:h-[400px] overflow-hidden">
            {testimonials.map((testimonial, index) => (
              <motion.div
                ...
                className={`absolute inset-0 bg-white rounded-lg md:rounded-xl lg:rounded-2xl shadow-md sm:shadow-lg p-5 sm:p-6 md:p-7 lg:p-8 flex flex-col ${index === currentIndex ? 'z-10' : 'z-0'}`}
              >
                <div className="flex items-center mb-3 sm:mb-4 md:mb-5">
                  ...
                  <div className="ml-2 sm:ml-3 md:ml-4">
                    <h3 className="text-sm sm:text-base md:text-lg lg:text-xl font-bold text-gray-800">{testimonial.name}</h3>
                    <p className="text-xs sm:text-sm md:text-base text-gray-600">{testimonial.role}</p>
                  </div>
                </div>

                <p className="text-xs sm:text-sm md:text-base lg:text-lg text-gray-700 mb-3 sm:mb-4 md:mb-5 lg:mb-6 flex-1">"{testimonial.content}"</p>
                ...
                <div className="flex items-center justify-between">
                  <div className="flex">
                    {[...Array(5)].map((_, i) => (
                      <StarIcon
                        key={i}
                        className={`h-3 w-3 sm:h-4 sm:w-4 md:h-5 md:w-5 ${i < testimonial.rating ? 'text-yellow-400 fill-yellow-400' : 'text-gray-300'}`}
                      />
                    ))}
                  </div>
                  <div className="text-cyan-600">
                    <svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" className="w-6 h-6 sm:w-7 sm:h-7 md:w-8 md:h-8 lg:w-10 lg:h-10">
          ...
          <div className="flex justify-center mt-4 sm:mt-6 md:mt-8 space-x-1.5 sm:space-x-2">
            {testimonials.map((_, index) => (
              <button
                ...
                className={`w-2 h-2 sm:w-2.5 sm:h-2.5 md:w-3 md:h-3 rounded-full transition-all ${index === currentIndex ? 'bg-cyan-600 sm:w-4 md:w-5' : 'bg-gray-300'}`}
        ...
        >
          {[
            ...
            >
              <span className="text-sm sm:text-base md:text-lg lg:text-xl mr-1 sm:mr-1.5 md:mr-2">{badge.icon}</span>
              <span className="font-medium text-gray-800 whitespace-nowrap">{badge.name}</span>
            </motion.div>
          ))}
        </motion.div>
      </div>
